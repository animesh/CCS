...
def train(population):
    ...
    root_path = "/fs/pool/pool-cox-data08/Juan/ccs/Data/two_populations"
    model_name = "density_most_intense"
    train_unique = pd.read_pickle(f"{root_path}/train_unique_{model_name}.pkl")
    ...
    path = "/fs/pool/pool-cox-data08/Juan/ccs/models/mobideep"
    cb = [tf.keras.callbacks.CSVLogger(f'{path}/training_{population}_{model_name}.log', append=False),  
        tf.keras.callbacks.ModelCheckpoint(f'{path}/checkpoints_{population}_{model_name}/best', save_best_only=True, save_weights_only=True),
...
if __name__ == "__main__":
    ...
    moodel, history = train("upper")
    model, history = train("lower")
# %%